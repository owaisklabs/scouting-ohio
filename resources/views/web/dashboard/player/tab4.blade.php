            ...
            <form action="{{ route('academic-info') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="col-lg-6 d-flex flex-column mt2 ">
                    ...
                    <div class="d-flex align-items-center">
                        <button type="button" class="uploadBtn uploadAble Transcript">Upload</button> <span
                            class="TranscriptSpan"></span>
                        <input type="file" name="transcript" class="TranscriptInp" value="" hidden>
                        @if ($errors->has('transcript'))
                            <div class="error rederror">{{ $errors->first('transcript') }}</div>
                        @endif
                    </div>
                </div>
                <div class="col-lg-6 d-flex flex-column mt2 ">
                    ...
                    <div class="d-flex align-items-center">
                        <button type="button" class="uploadBtn uploadAble aTTACHact">Upload</button> <span
                            class="aTTACHactSpan"></span>
                        <input type="file" name="attach_act_sat_result" class="aTTACHactInp" hidden>
                        @if ($errors->has('attach_act_sat_result'))
                            <div class="error rederror">{{ $errors->first('attach_act_sat_result') }}</div>
                        @endif
                    </div>
                </div>
                <div class="col-lg-6 d-flex flex-column mt2 ">
                    ...
                    <div class="d-flex align-items-center">
                        <button type="button" class="uploadBtn uploadAble fOOTbALLbIO">Upload</button> <span
                            class="fOOTbALLbIOSpan"></span>
                        <input type="file" name="football_bio" class="fOOTbALLbIOInp" hidden>
                        @if ($errors->has('football_bio'))
                            <div class="error rederror">{{ $errors->first('football_bio') }}</div>
                        @endif
                    </div>
                </div>
                <div class="col-lg-6 d-flex flex-column mt2">
                    <label>DESIRED MAJOR IN COLLEGE:</label>
                    <input type="text" value="{{ @$academicInfo->desire_majro_in_college }}"
                        name="desire_majro_in_college" id="">
                </div>
                <div class="col-lg-6 d-flex flex-column mt2">
                    <label>sat:</label>
