    ...
    <div class="userTableCustom mt4">
        ...
        <div class="tableRow d-flex mt1 mb2" style="padding: 20px;">
            ...
            <div class="actions d-flex saveADmin align-items-center">
                <button  class="editButton" data-id="{{$addtionalCoach->id}}"  data-bs-toggle="modal" data-bs-target="#editexampleModalCOAH">EDIT</button>
...
<script>
    ...
    const displayData = (data)=>{
            $('#editexampleModalCOAH .modal-body [name=name]').val(data.name)
            $('#editexampleModalCOAH .modal-body [name=email]').val(data.email)
            $('#editexampleModalCOAH .modal-body [name=id]').val(data.id)
    }
    ...
    $.ajax({
        url: '<?php echo(route("delete-addtional-coach-by-id")); ?>',
        type: 'GET',
        data: {
            "id": id,
            "_token": token,
